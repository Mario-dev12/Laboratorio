...
payment_methodRepository.readPayment_methodByName = async text => {
  try {
    ...
    const resp = await pool.query(`SELECT * FROM sp_find_answer_by_name('${text}')`);
...
payment_methodRepository.createPayment_method = async (name, amount, bank, type, idExam) => {
  try {
    ...
    const resp = await pool.query(`SELECT * FROM sp_create_answer('${name}', '${amount}', '${bank}', '${type}', ${idExam})`);
...
payment_methodRepository.updatePayment_method = async (id, answer) => {
  try {
    ...
    const resp = await pool.query(`SELECT * FROM sp_update_answer(${id}, '${answer.name}', '${answer.amount}', '${answer.bank}', '${answer.type}', ${answer.idExam})`);
