...
usersRepository.createUser = async (ci, passport, firstName, lastName, genre, age, address, phone, email, doctor) => {
	try {
		const resp = await pool.query(
			`SELECT * FROM sp_create_users('${ci}', '${passport}', '${firstName}', '${lastName}', '${genre}', ${age}, '${address}', '${phone}', '${email}', '${doctor}')`
		);
...
usersRepository.updateUser = async (id, answer) => {
	try {
		const resp = await pool.query(
			`SELECT * FROM sp_update_users(${id}, '${answer.ci}', '${answer.passport}', '${answer.firstName}', '${answer.lastName}', '${answer.genre}', ${answer.age}, '${answer.address}', '${answer.phone}', '${answer.email}', '${answer.doctor}')`
		);
