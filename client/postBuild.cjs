...
fs.readFile(filePath, "utf8", (err, data) => {  
    ...
    const legacyChunkRegex = /<script nomodule crossorigin id="vite-legacy-entry" data-src="(.*?\/assets\/index-legacy-)(.*?)">.*?<\/script>/; // Captura el <script>  
    ...
    const newCssFile = cssFileMatch  
        ? `<link rel="stylesheet" crossorigin href="${path.resolve(__dirname, 'dist', cssFileMatch[1] + '/assets/index-DR98lWJv.css')}">`  
        : '';  
