			...
			<div class="container">
				<ion-tabs v-if="tabNames.length > 0">
					<ion-tab-bar slot="top">
						<ion-tab-button v-for="(tabName, index) in tabNames" :key="index" :tab="tabName" @click="loadTabResults(tabName)">
							<ion-label>{{ tabName }}</ion-label>
						</ion-tab-button>
					</ion-tab-bar>

					<ion-tab v-for="(tabName, index) in tabNames" :key="index" :tab="tabName">
						<ion-content>
									...
									<table v-if="currentResults.firstTable.length > 0">
										...
										<tbody>
											<tr v-for="(item, idx) in currentResults.firstTable" :key="idx">
												<td>{{ item.nombre }}</td>
												<td>
													<input type="text" v-model="item.value" placeholder="Escribe un valor" class="input-field" />
												</td>
												<td>{{ item.unidad }}</td>
											</tr>
										</tbody>
...
<script setup lang="ts">
	...
	const loadTabResults = (tabName: string) => {
		const results =
			profileData.value[tabName]?.resultado.map((item: any) => ({
				...item,
				value: "",
			})) || [];
