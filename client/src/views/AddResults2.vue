				...
				<div ref="profileRef" id="profile">
					...
					<div class="profile-content" v-for="(profile, indx) in profilesData" :key="indx" ref="profileRef2">
						<div class="profile-sections mt-1 size" v-show="showProfile[indx]">
							<div class="profile-tables">
								<div class="table-responsive">
									<table class="table table-hover table-striped m-0">
										...
										<tbody class="testTitleTbody">
											<tr class="text-center">
												<td class="p-0" colspan="4">
													<h4 class="test-title m-0 text-nowrap title-size">{{ profileNamesOrdered[indx] }}</h4>
												</td>
											</tr>
										</tbody>
										<tbody
											...
											<tr class="text-center">
												<td class="p-0" colspan="4">
													<h5 class="section-title m-0 text-nowrap subtitle-size">{{ key }}</h5>
												</td>
											</tr>
							...
							<div class="currentProfile-btn d-flex justify-content-center mt-3" style="gap: 10px">
								<button class="btn btn-primary" @click="printCurrentProfile(profileRef2[indx])">Imprimir este perfil</button>

								<button class="btn btn-primary" @click="printProfileToPDF(profileRef2[indx])">Imprimir por partes</button>
							</div>
						</div>
					</div>
				</div>
...
<script setup lang="ts">
	...
	onMounted(async () => {
		...
		today.value = new Date(ordersArray.value[0].createdDate);

		day.value = today.value.getDate();
		...
		const primarySectionsStrings: string[] = [
			...
			"PT",
			"PTT",
			"UROANÁLISIS - ANÁLISIS FÍSICO",
			"UROANÁLISIS - ANÁLISIS FISICO",
			"UROANÁLISIS - ANÁLISIS QUÍMICO",
			"UROANÁLISIS - ANÁLISIS MICROSCÓPICO",
		];
		...
		for (const profile of ordersArray.value) {
			...
			// Verifica si la sección 'Análisis Químico' existe en los datos
			const searchText = "análisis químico"; // El texto a buscar, puedes ponerlo en minúsculas para simplificar

			// La solución de una sola línea (funcionalmente)
			const foundKey = Object.keys(profileSection2).find((key) => key.toLowerCase().includes(searchText.toLowerCase()));

			// Si 'foundKey' existe (no es undefined), se usa como índice; de lo contrario, 'quimicoSection' será 'null'
			const quimicoSection = foundKey ? profileSection2[foundKey] : null;

			if (quimicoSection && quimicoSection.resultado) {
				// Recorre los resultados de la sección de Análisis Químico
				quimicoSection.resultado.forEach((item: { valor: string }) => {
					// Si el valor está vacío, se le asigna 'NEGATIVO'
					if (item.valor === "") {
						item.valor = "NEGATIVO";
					}
				});
	...
	router.beforeEach(async (to, from, next) => {
		...
		if (to.name === "Results2") {
			...
			for (const profile of ordersArray.value) {
				...
				// Usamos el encadenamiento opcional para la clave y asignamos null si no se encuentra
				const quimicoSection = foundKey ? profileSection2[foundKey] : null;
			...
			console.log(profileNamesOrdered);
			...
			profilesData.value = newSections;
			console.log("profilesData.value", profilesData.value);
		...
		if (profileRef2.value) {
			...
			await Promise.all(
				profileRef2.value.map(async (item: any) => {
					...
					const profileTitleElement = item.querySelector("h4.test-title");
	...
	async function mergeTables(element: HTMLElement) {
		...
		console.log("html element", clonedElement);
		...
		//Goes through all values/campos and applies the text formatter regexSufijos
		...
		//This forEAch finds and modifies the values for hematies and erotrocitos inputs.
		tables.forEach((table) => {
		...
		//Goes through all inputs of each section and formats the value of the input
		tables.forEach((table) => {
			const filasDeDatos = table.querySelectorAll("tr.rowData");
			filasDeDatos.forEach((fila: Element) => {
				const celdaInputElement = fila.querySelector("td.inputElement");
		...
		let ptTitleTbody: HTMLTableSectionElement | undefined;
		let ptSectionTbody: HTMLTableSectionElement | undefined;
		let pttTitleTbody: HTMLTableSectionElement | undefined;
		let pttSectionTbody: HTMLTableSectionElement | undefined;

		//Here it loops through all tbody elements in each table to find the pt & ptt tbody elements and asigns each to a variable
		tables.forEach((table) => {
			const allTbodiesInTable = Array.from(table.querySelectorAll("tbody")) as HTMLTableSectionElement[];
			for (let i = 0; i < allTbodiesInTable.length; i++) {
				const currentTbody = allTbodiesInTable[i];
				const titleElement = currentTbody.querySelector("h4");
				const nextTbody = allTbodiesInTable[i + 1];

				if (titleElement && normalizeText(titleElement.textContent) === "pt" && nextTbody) {
					ptTitleTbody = currentTbody;
					ptSectionTbody = nextTbody;
				} else if (titleElement && normalizeText(titleElement.textContent) === "ptt" && nextTbody) {
					pttTitleTbody = currentTbody;
					pttSectionTbody = nextTbody;
				}
			}
		});

		//This is moving all tbody elements of each table to the first table
		if (tables.length > 0) {
		...
		//este if esta borrando el uroanalisis
		if (tables.length > 0) {
			const firstTable = tables[0];
			...
			//Here it will only return a tbody to tbodyToPlace if it matches one of the main section names. Thats wrong!!
			...
			if (ptTitleTbody && ptSectionTbody) {
				sortedTbodies.push(ptTitleTbody);
				processedTbodies.add(ptTitleTbody);
				sortedTbodies.push(ptSectionTbody);
				processedTbodies.add(ptSectionTbody);
			}

			if (pttTitleTbody && pttSectionTbody) {
				sortedTbodies.push(pttTitleTbody);
				processedTbodies.add(pttTitleTbody);
				sortedTbodies.push(pttSectionTbody);
				processedTbodies.add(pttSectionTbody);
			}

			// Se filtra el resto de los tbodies, asegurando que solo se incluyan los que no han sido procesados
			// y que no sean un título de perfil no deseado (como "Perfil 20").

			allTbodies.forEach((tbody) => tbody.remove());
			sortedTbodies.forEach((tbody) => firstTable.appendChild(tbody));
			console.log(sortedTbodies);
		}

		//Here it goes through all table sections and finds the main sections and adds them to primaryMainSectionTbodiesMap
		...
		//Goes through all sections of an exam, finds duplicate primary sections and adds the rows to the corresponding
		//primary section in primaryMainSectionTbodiesMap
		tables.forEach((table) => {
			const sectionsInTable = Array.from(table.querySelectorAll("tbody.sectionData")) as HTMLTableSectionElement[];
		...
		// If all inputs are empty erase that tbody
		tables.forEach((table) => {
		...
		//Moves all sections to the first table
		if (tables.length > 0) {
			const firstTable = tables[0];
			for (let i = 1; i < tables.length; i++) {
				const currentTable = tables[i];
		...
		//Here it organizes the sections and adds them to the first table/ merged table
		//Note: tbody = exam section
		if (tables.length > 0) {
			const firstTable = tables[0];
			const allTbodies = Array.from(firstTable.querySelectorAll("tbody")) as HTMLTableSectionElement[];
			const sortedTbodies: HTMLTableSectionElement[] = [];
	...
	async function mergeTables2(element: HTMLElement) {
		const tables = Array.from(element.querySelectorAll("table"));
		const profileSection = element.querySelector(".profile-sections") as HTMLElement;
		...
		//Este if hace que desde cualquier pestaña de un perfil se pueda crear el pdf
		//Si no se tiene y se intenta crear el pdf desde una pestaña que no sea la primera no sale nada en el pdf porque
		//la primera profileSection tendria display none.
		if (profileSection) {
			profileSection.style.display = "block";
		}
		...
		tables.forEach((table) => {
			...
			//Encuentra y multiplica los valores de hematies y erotrocitos
			//Refatorizar este codigo!!!
			const seccionesData = Array.from(table.querySelectorAll("tbody.sectionData"));
			seccionesData.forEach((tbody) => {
				const tituloSeccionElemento = tbody.querySelector("tr:first-child td h5");
			...
			//Pasar por cada examen y borrar titulos de un perfil que tenga por lo menos una seccion primaria
			let testTitleTbody: HTMLElement | null = table.querySelector("tbody.testTitleTbody");
			const testTitle = table.querySelector("h4.test-title");
			...
			if (
				...
				testTitle.textContent.toLowerCase().includes("perfil")
			) {
				testTitleTbody.remove();
			}

			//Update the testTileTbody variable so it doesn't get added again after erased
			testTitleTbody = table.querySelector("tbody.testTitleTbody");

			//Encontrar examenes pt, ptt, uroanalisis y coproanalisis y guardar su titulo y secciones en su respectiva variable
			const sectionsData = table.querySelectorAll("tbody.sectionData");

			switch (normalizeText(testTitle?.textContent)) {
				case "pt": {
	...
	const pdfWithoutSignature = async () => {
		...
		await mergeTables2(profileRefCopy);
		...
		const observacionesText = contentDivCloned.querySelector("#observaciones") as HTMLTextAreaElement;
		if (observacionesText.value) {
			const observacionesDiv = contentDivCloned.querySelector(".Observaciones") as HTMLElement;
			paginaActual.appendChild(observacionesDiv);
		} else if (observaciones && !observacionesText.value) {
			console.log("remove observaciones");
			const observacionesDiv = contentDivCloned.querySelector(".Observaciones") as HTMLElement;
	...
	async function printProfileToPDF(currentProfile: HTMLElement) {
		...
		// Seleccionar todos los elementos tbody dentro de .profile-tables
		const allTbodys = currentProfileCopy.querySelectorAll(".profile-tables tbody");

		let uroanalisisTbodyIndex = -1;
		let coproanalisisTbodyIndex = -1;

		// Encontrar el índice del primer tbody que contiene el título "uroanálisis" o "coproanálisis"
		for (let i = 0; i < allTbodys.length; i++) {
			const tbody = allTbodys[i];
			const titleElement = tbody.querySelector("h5.subtitle-size");
			const titleText = titleElement?.textContent?.toLocaleLowerCase().trim();

			// Si encontramos "uroanálisis" y aún no hemos registrado su índice, lo guardamos.
			if (uroanalisisTbodyIndex === -1 && titleText?.includes("uroanálisis")) {
				uroanalisisTbodyIndex = i;
			}

			// Si encontramos "coproanálisis" y aún no hemos registrado su índice, lo guardamos.
			if (coproanalisisTbodyIndex === -1 && titleText?.includes("coproanálisis")) {
				coproanalisisTbodyIndex = i;
			}
		}

		// Si se encontró el tbody de Uroanálisis y no es el primer tbody del documento
		if (uroanalisisTbodyIndex > 0) {
			const prevTbody = allTbodys[uroanalisisTbodyIndex - 1] as HTMLElement;
			if (prevTbody) {
				prevTbody.classList.add("html2pdf-pagebreak");
			}
		}

		// Si se encontró el tbody de Coproanálisis y no es el primer tbody del documento
		if (coproanalisisTbodyIndex > 0) {
			const prevTbody = allTbodys[coproanalisisTbodyIndex - 1] as HTMLElement;
			if (prevTbody) {
				prevTbody.classList.add("html2pdf-pagebreak");
			}
		}

		// --- Fin de la lógica de salto de página mejorada ---
		...
		const options = {
			...
			// Aquí se define la clase para el salto de página
			pagebreak: { mode: "css", after: ".html2pdf-pagebreak" },
		};
	...
	async function printCurrentProfile(currentProfile: HTMLElement) {
		...
		console.log("llega", currentProfileCopy);
