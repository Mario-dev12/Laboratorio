<template>
	<ion-modal :is-open="isOpen" @did-dismiss="closeModal">
		<ion-header>
			<ion-toolbar>
				<ion-title>Editar Antibiotico</ion-title>
		...
		<ion-content>
			<form @submit.prevent="submit">
				<div class="form-group">
					<label class="form-label">Nombre del Antibiotico</label>
					<input
						v-model="form.name"
						type="text"
						required
						class="form-control custom-input"
						placeholder="Ingrese el nombre del antibiotico" />
				</div>
				<ion-footer>
	...
	watch(
		...
		(newUser) => {
			if (newUser) {
				...
				form.value.name = newUser.nombre;
				form.value.idAntibiotico = newUser.idAntibiotico;
			}
		}
	);
