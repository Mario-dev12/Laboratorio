<template>
	<ion-modal :is-open="isOpen" @did-dismiss="closeModal">
		<ion-header>
			<ion-toolbar>
				<ion-title>Editar Bacteria</ion-title>
		...
		<ion-content>
			<form @submit.prevent="submit">
				<div class="form-group">
					<label class="form-label">Nombre de la bacteria</label>
					<input
						v-model="form.name"
						type="text"
						required
						class="form-control custom-input"
						placeholder="Ingrese el nombre de la bacteria" />
				</div>
				<ion-footer>
	...
	watch(
		...
		(newUser) => {
			if (newUser) {
				...
				form.value.idBacteria = newUser.idBacteria;
			}
		}
	);
